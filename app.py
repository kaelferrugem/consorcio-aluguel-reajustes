...
# CSS para garantir visibilidade e estilo
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    # VARIÁVEL: LANCE EMBUTIDO (Mantendo o crédito nominal fixo)
...
def rodar_simulacao():
    ...
    # 2. CONSÓRCIO (Lógica: Crédito Nominal Fixo)
    credito_nom = v_imovel # O contrato é feito sobre o valor atual do imóvel
    p_cons = (credito_nom * (1 + taxa_adm)) / prazo_cons
    reserva = entrada_fin - lance_proprio # Capital que o cliente "guardou"
    ...
    for m in range(1, prazo_fin + 1):
        ...
        # Valorização do imóvel de mercado
        ...
        if m == mes_contemplacao:
            ...
            # O cliente usa o líquido + o lance próprio para comprar.
            # Se faltar para o valor de mercado, ele tira da reserva acumulada.
            ...
            s_devedor_c -= v_embutido # Abate a dívida com o lance embutido
        
        if imovel_c > 0: 
            imovel_c *= (1 + v_mensal)
            
        reserva *= (1 + s_mensal) # A reserva continua rendendo
...
c1, c2 = st.columns(2)
with c1: st.metric("Patrimônio com Financiamento", f"R$ {res_fin['Patrimônio']:,.2f}")
with c2: st.metric("Patrimônio com Consórcio", f"R$ {res_con['Patrimônio']:,.2f}")

c3, c4 = st.columns(2)
with c3: st.metric("Custo Total Financiamento", f"R$ {res_fin['Custo Acumulado']:,.2f}")
with c4: st.metric("Custo Total Consórcio + Aluguel", f"R$ {res_con['Custo Acumulado']:,.2f}")
